...
export class AutomationEngine {
  ...
  async getContext(account: BookmakerAccount): Promise<BrowserContext> {
    const contextKey = account.id;
    if (this.contexts.has(contextKey)) return this.contexts.get(contextKey)!;
    if (!this.browser) await this.init();

    const sessionFile   = path.join(SESSIONS_DIR, `${account.id}_${account.bookmaker}.json`);
  ...
  async saveSession(account: BookmakerAccount) {
    const ctx = this.contexts.get(account.id);
    if (!ctx) return;
    await ctx.storageState({ path: path.join(SESSIONS_DIR, `${account.id}_${account.bookmaker}.json`) });
  }
    ...
    return this.getMutex(account.bookmaker).run(async () => {
      ...
      try {
        ...
        if (alreadyIn) {
          ...
          await page.close();
          this.setStatus(account.id, { state: 'ready', message: 'Sessão ativa', balance: balance ?? undefined });
        ...
        if (ok) {
          ...
          await page.close();
          this.setStatus(account.id, { state: 'ready', message: 'Login realizado', balance: balance ?? undefined });
    ...
    return this.getMutex(bookmaker).run(async () => {
      ...
      for (let attempt = 1; attempt <= maxRetries; attempt++) {
        ...
        try {
          ...
          this.setStatus(account.id, { state: 'cooldown', message: `Aguardando ${Math.round(backoff/1000)}s antes de tentar novamente...` });
          ...
          if (attempt < maxRetries) {
            const backoff = Math.pow(3, attempt) * 5000;
            this.setStatus(account.id, { state: 'cooldown', message: `Erro de rede. Aguardando ${Math.round(backoff/1000)}s...` });
            await humanDelay(backoff, backoff * 0.1);
          }
        }
      }
  ...
  // ── Bet Queue ───────────────────────────────────────────────
  addToQueue(accountId: string, bookmaker: string, order: BetOrder, maxRetries = 3, ownerUserId?: number): QueueItem {
  ...
  async runQueue(accounts: BookmakerAccount[], ownerUserId?: number): Promise<QueueItem[]> {
  ...
  // ── Status ──────────────────────────────────────────────────
  private setStatus(key: string, status: AutomationStatus) {
    this.status.set(key, { ...status, updatedAt: new Date().toISOString() });
  }

  getStatus(key: string): AutomationStatus | null { return this.status.get(key) ?? null; }
  getAllStatus(): Record<string, AutomationStatus> { return Object.fromEntries(this.status.entries()); }

  // ── Logging ─────────────────────────────────────────────────
  private logBet(bookmaker: string, order: BetOrder, result: BetResult, accountId: string, ownerUserId?: number) {
  ...
  getLogs(days = 7, ownerUserId?: number, accountIds?: string[]): unknown[] {
    ...
    return entries
      .filter((entry) => ownerUserId == null || Number(entry.ownerUserId ?? -1) === ownerUserId)
      .filter((entry) => !accountIds?.length || accountIds.includes(String(entry.accountId ?? '')))
      .reverse();
  }
}
